...
public class TaskPanel extends JPanel {
    ...
    public TaskPanel(KidTaskMain mainFrame) {
        ...
        model = new DefaultTableModel(cols, 0) {
            ...
            public Class<?> getColumnClass(int columnIndex) {
                // Puan sütunu sayısal sıralansın diye Integer döndürüyoruz
        ...
        table.getColumnModel().getColumn(6).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                if (stars > 0) {
                    ...
                    star.setForeground(new Color(255, 193, 7)); // Altın sarısı
    ...
    private void addTask() {
        ...
        TaskDialog dialog = new TaskDialog(mainFrame);
        ...
        if (dialog.isConfirmed()) {
            try {
                ...
                mainFrame.showNotification("New Task Created Successfully! ✨", "SUCCESS");

            } catch (Exception e) {
                mainFrame.showNotification("Error creating task.", "ERROR");
            }
        }
    }
    ...
    private void processTask() {
        ...
        if (row == -1) {
            mainFrame.showNotification("Please select a task first!", "ERROR");
            return;
        }
        ...
        if ("CHILD".equals(role)) {
            if ("PENDING".equals(task.getStatus())) {
                ...
                mainFrame.showNotification("Task Sent for Approval! Good Job! 🌟", "SUCCESS");
            } else {
                mainFrame.showNotification("This task is already completed!", "WARNING");
            }
        }
        else {
            if ("COMPLETED".equals(task.getStatus())) {
                ...
                if (result != null) {
                    try {
                        ...
                        // EBEVEYN ONAY BİLDİRİMİ
                        mainFrame.showNotification("Task Approved! Points Added! ✅", "SUCCESS");
                    } catch (NumberFormatException e) {
                        mainFrame.showNotification("Invalid rating number!", "ERROR");
                    }
                }
            } else {
                mainFrame.showNotification("Only completed tasks can be approved.", "WARNING");
            }
        }
        DataManager.saveData();
        refreshTable();
    ...
    private void deleteTask() {
        ...
        refreshTable();
        mainFrame.updateDashboardStats();
        mainFrame.showNotification("Task Deleted Successfully.", "WARNING");
    }
}